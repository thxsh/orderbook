...
//! # Run
//!
//! `RUST_LOG=info cargo run --bin --features="daemon-bin" local_daemon --package "orderbook-standalone"`
use orderbook_standalone::ORDERBOOK_STANDALONE_ID;
...
use orderbook_standalone::{msg::OrderbookStandaloneInstantiateMsg, OrderbookStandaloneInterface};
...
fn main() -> anyhow::Result<()> {
    ...
    let standalone_namespace = Namespace::from_id(ORDERBOOK_STANDALONE_ID)?;
    ...
    publisher.publish_standalone::<OrderbookStandaloneInterface<Daemon>>()?;
    ...
    let standalone = account.install_standalone::<OrderbookStandaloneInterface<_>>(
