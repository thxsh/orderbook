...
struct TestEnv<Env: CwEnv> {
    ...
    adapter: Application<Env, OrderbookAdapterInterface<Env>>,
}
...
impl TestEnv<MockBech32> {
    ...
    fn setup() -> anyhow::Result<TestEnv<MockBech32>> {
        ...
        publisher.publish_adapter::<OrderbookAdapterInstantiateMsg, OrderbookAdapterInterface<_>>(
            OrderbookAdapterInstantiateMsg {},
        )?;

        let adapter = publisher
            .account()
            .install_adapter::<OrderbookAdapterInterface<_>>(&[])?;
...
fn update_config() -> anyhow::Result<()> {
    ...
    let publisher_account = env
        .abs
        .publisher_builder(Namespace::new(ORDERBOOK_NAMESPACE).unwrap())
        .build()?;

    adapter.execute(
        &AdapterRequestMsg {
            proxy_address: Some(publisher_account.account().proxy()?.to_string()),
            request: OrderbookAdapterExecuteMsg::UpdateConfig {},
        }
        .into(),
        None,
    )?;
    ...
    let config = adapter.config()?;
    let expected_response = orderbook_adapter::msg::ConfigResponse {};
...
fn set_status() -> anyhow::Result<()> {
    ...
    subaccount.as_ref().manager.execute_on_module(
        ...
        ExecuteMsg::Module(AdapterRequestMsg {
            ...
            request: OrderbookAdapterExecuteMsg::SetStatus {
    ...
    let new_account = env
        .abs
        .account_builder()
        .install_adapter::<OrderbookAdapterInterface<MockBech32>>()?
        .build()?;
